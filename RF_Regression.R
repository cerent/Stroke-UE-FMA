...
# tune the hyperparameters using the train dataset (10 fold CV and repeat the CV 10 times)                          
tuneResult<-tune.randomForest(x=trainData[,-1],y=trainData[,1], data = NULL,mtry = seq(3,10,1), ntree = seq(10,100,10),center = TRUE, scale = TRUE,
tunecontrol=tune.control(sampling = "cross",cross = 10,fix = 1/10,nrepeat = 10))
...
var_imp<-as.matrix(tunedModel$importance[,1])   


# Save the best hyperparameters and the results in a matrix
result_cross_validation[outerloop,1]<-tuneResult$best.parameters$ntree
result_cross_validation[outerloop,2]<-tuneResult$best.parameters$mtry
...
var_imp_matrix<-cbind(var_imp_matrix,as.matrix(var_imp))
